...
if($_SESSION['nivell_acces_ctel']==1 || $_SESSION['nivell_acces_ctel']==2){
	...
	$elimina = mysql_query("DELETE FROM categorias WHERE id = $id ", $link);
	if (!$elimina) {
		die("No s'ha pogut eliminar la categoria sel·lecionada de la base de dades: " . mysql_error());
	}	
	include('../DB/connexio-tanca-db.php');
	header ("Location: gestio_categories.php");
